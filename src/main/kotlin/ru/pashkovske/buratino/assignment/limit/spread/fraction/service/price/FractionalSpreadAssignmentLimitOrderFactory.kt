package ru.pashkovske.buratino.assignment.limit.spread.fraction.service.price
...
@Service
class FractionalSpreadAssignmentLimitOrderFactory(
    private val marketPriceService: MarketPriceService,
    private val instrumentService: InstrumentService
) : BasicLimitOrderFactory<FractionalSpreadAssignment>() {

    private val log: KLogger = KotlinLogging.logger {}

    override fun getPrice(assignment: FractionalSpreadAssignment): Price {
        val instrument: ru.pashkovske.buratino.instrument.model.Instrument = instrumentService.get(assignment.iid)
        val step: Price = instrument.minPriceIncrement
        val directTopPrice: Price = marketPriceService.getOneStepOverTopOfBook(
            iid = assignment.iid,
            direction = assignment.direction
        )!!
        val oppositeTopPrice: Price = marketPriceService.getTopOfBook(
            iid = assignment.iid,
